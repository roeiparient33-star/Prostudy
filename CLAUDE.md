...
- `src/lib/markdown.js` — `renderMarkdown`: Markdown→HTML עם KaTeX, צביעת תחביר (class), טבלאות, SVG ו-Mermaid.
...
1. **הצ'אט לא מציג את תוכן התשובה** — רק כרטיס קומפקטי "הסיכום מוכן" עם כפתור הורדה. כל התוכן יורד כ-PDF.
2. **כל יופי ה-PDF נעשה בצד הלקוח** (CSS + highlight.js/mermaid/KaTeX מ-CDN בחלון ההדפסה) — **אפס טוקנים**. אסור לבקש מ-Claude לייצר HTML מעוצב.
3. **ויזואלים**: Claude מונחה לצייר **SVG** (גרפים/צורות) או **Mermaid** (תרשימים) — לעולם לא ASCII art.
...
- **Prompt caching**: למסמך ול-`SYSTEM_PROMPT` יש `cache_control: ephemeral` (TTL 5 דק'). **כל שינוי בייט ב-SYSTEM_PROMPT שובר את ה-cache** — אל תוסיף תאריך/שם משתמש שם.
