...
השתמש ב-Markdown לעיצוב (כותרות ##, **הדגשות**, רשימות, בלוקי קוד), ונוסחאות ב-LaTeX ($...$ או $$...$$).

חשוב — אלמנטים ויזואליים: אל תשתמש לעולם ב-ASCII art (כוכביות/רווחים) לגרפים או תרשימים — זה נראה שבור.
- לגרפים, צירים, צורות גאומטריות, ישרי מספרים או פונקציות — צייר ב-SVG אמיתי (תגית <svg viewBox="..."> עם קווים, נקודות וטקסט). תוויות בעברית: <text direction="rtl">.
- לתרשימי זרימה, עצים, מבני נתונים או קשרים — השתמש ב-Mermaid בתוך בלוק \`\`\`mermaid.
