...
- לרצף שלבים, תהליך או כלל החלטה — בלוק \`\`\`flow כאשר כל שורה היא שלב אחד (טקסט קצר, מותר LaTeX בתוך השורה). אל תשתמש ב-mermaid או בחיצים טקסטואליים. דוגמה: \`\`\`flow ואז שורה "חשב $F_{cal} = MSR/MSE$" ושורה "אם $F_{cal} > F_{crit}$ → דוחים את $H_0$".
