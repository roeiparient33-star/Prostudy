...
function saveEvents(evs) { localStorage.setItem(LS_KEY, JSON.stringify(evs)); }
...
function fmtShort(date) { return `${date.getDate()}.${date.getMonth() + 1}`; }
...
function weekRangeLabel(dates) {
  ...
  return `${s.getDate()}–${e.getDate()} ב${month}`;
}
...
export default function Schedule() {
  ...
  const tasksByDay = useMemo(() => {
    ...
    weekDates.forEach((date, i) => {
      ...
      map[i] = tasks.filter(t => !t.completed && t.dueDate === iso);
    });
    return map;
  }, [tasks, weekDates]);
  ...
  const nowTop = ((nowMin - HOURS[0] * 60) / 60) * CELL_H;
  const showNow = isThisWeek && nowTop >= 0 && nowTop <= TOTAL_H;
    ...
    <div className="page-enter">
      ...
      <div className="sch-week-nav">
        ...
        <div className="sch-week-label">
          {isThisWeek
            ? <><span className="sch-week-today-dot"/>השבוע</>
            : weekRangeLabel(weekDates)
          }
        </div>
        {!isThisWeek && (
          <button className="sch-today-btn" onClick={() => setWeekOffset(0)}>
            השבוע
          </button>
        )}
      ...
      <div className="card schedule-card">
        <div className="sch-wrap">
          ...
          <div className="sch-body">
            ...
            {DAYS.map((_, di) => {
              const isToday = isThisWeek && di === todayIdx;
              return (
                <div
                  key={di}
                  className={`sch-col${isToday ? ' sch-col-today' : ''}`}
        ...
        {events.length === 0 && (
          <div className="schedule-empty">
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-2)', marginBottom: 6 }}>המערכת ריקה</div>
  ...
  return (
    <div style={{ display:'flex', gap:6, alignItems:'flex-end', direction:'ltr' }}>
      <div style={{ display:'flex', flexDirection:'column' }}>
        ...
        <select className="modal-input" style={{ width:68, paddingInline:6, textAlign:'center' }} value={h}
      ...
      <div style={{ display:'flex', flexDirection:'column' }}>
        ...
        <select className="modal-input" style={{ width:68, paddingInline:6, textAlign:'center' }} value={nearestMin}
...
function AddEventModal({ defaultDay, defaultHour, courses, onAdd, onClose }) {
  ...
  function pickCourse(course) { setTitle(course.name); setColor(course.color); }
      ...
      <div className="modal-box">
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          {courses.length > 0 && (
            <div className="modal-field">
              ...
              <div className="sch-course-chips">
                {courses.map(c => (
                  <button key={c.id} type="button" className="sch-course-chip"
          ...
          <div className="modal-row-2">
            <div className="modal-field"><label className="modal-label">התחלה</label><TimePicker value={startTime} onChange={setStartTime}/></div>
            <div className="modal-field"><label className="modal-label">סיום</label><TimePicker value={endTime} onChange={setEndTime}/></div>
          </div>
          <div className="modal-row-2">
            <div className="modal-field">
